...
No keyless snapshot lane exercises a pi-ai route: the recorded-session corpus drives `dsh-llm-replay`, which declares modalities directly in its configuration, and a pi-ai route needs a live endpoint whose port a static `cordis.yml` cannot name. The admission points this change feeds are covered by `read-image` and `read-image-text-route` and are unaffected — what changed is what one adapter reports, not how a gate reads it.
